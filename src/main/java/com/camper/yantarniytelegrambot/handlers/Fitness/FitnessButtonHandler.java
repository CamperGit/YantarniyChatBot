...
public class FitnessButtonHandler implements BotButtonHandler {
    ...
    public List<PartialBotApiMethod<?>> openCoachesMenu(String chatId, CallbackQuery query) {
        return new ArrayList<>(Collections.singletonList(Utils.changeMessage(localeMessageSource.getMessage("onAction.fitnessCoachesButton"),
                chatId,
                query.getMessage().getMessageId(),
                getCoachesMenuMarkup())));
    }

    public List<PartialBotApiMethod<?>> openSchedulesMenu(String chatId, CallbackQuery query) {
        return new ArrayList<>(Collections.singletonList(Utils.changeMessage(localeMessageSource.getMessage("onAction.fitnessScheduleButton"),
                chatId,
                query.getMessage().getMessageId(),
                getSchedulesMenuMarkup())));
    }
